...
function reviewsadv_list(id_review,action,value,token){

    if(action == 'active') {
        $('#activeitem' + id_review).html('<img src="../img/admin/../../modules/reviewsadv/views/img/loader.gif" />');
    }
            ...
            if (data.status == 'success') {
                ...
                if(action == 'abuse' || action == 'changed') {
                    ...
                    var html = '<span class="label-tooltip" data-original-title="Click here to activate or deactivate review on your site" data-toggle="tooltip">'+
                            '<a href="javascript:void(0)" onclick="reviewsadv_list('+id_review+',\'active\', '+action_value+',\'\');" style="text-decoration:none">'+
                        '<img src="../img/admin/../../modules/reviewsadv/views/img/'+img_ok+'.gif" />'+
                        '</a>'+
                    '</span>';
...
function initCustomersAutocomplete14_13(){
    $('document').ready( function() {
        $('#customer_autocomplete_input')
            .autocomplete('../modules/reviewsadv/backward_compatibility/ajax-tab14_13.php',{
                minChars: 1,
                max: 20,
                width: 500,
                selectFirst: false,
        ...
        $.post('../modules/reviewsadv/reviews_admin.php', {
            ...
            function (data) {
                if (data.status == 'success') {
                    ...
                    $('.avatar-form').html('');
                    $('.avatar-form').html('<img src = "../modules/reviewsadv/views/img/avatar_m.gif" />');
...
function delete_file(item_id){
    if(confirm("Are you sure you want to remove this item?"))
    {
        ...
        $.post('../modules/reviewsadv/upload.php', {
                action:'deletefile',
                item_id : item_id
            },
            function (data) {
