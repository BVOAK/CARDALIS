...
{if $reviewsadvis_blocklr == 1 && $reviewsadvblocklr_chook == "blocklr_chook"}
...
<div id="reviewsadv_block_left" class="block-last-reviewsadvs block blockmanufacturer" style="width:{$reviewsadvblocklr_w|escape:'htmlall':'UTF-8'}%">
		...
		<div class="block_content block-items-data row-custom">
			{if count($reviewsadvreviews_block_chook)>0}


			{foreach from=$reviewsadvreviews_block_chook item=review name=myLoop}

                <div class="items-last-reviewsadvs ">
                {if $review.product_img}
                    <div class="img-block-reviewsadv col-sm-1-custom">
                ...
                {/if}
                <div class="body-block-reviewsadv col-sm-11-custom {if !$review.product_img}body-block-reviewsadv-100{/if}">
                    <div class="title-block-last-reviewsadv">


                    <div class="r-product">
                        {if strlen($review.customer_name)>0}
                            {l s='By' mod='reviewsadv'}&nbsp;<strong>{$review.customer_name|escape:'html':'UTF-8'}</strong>
                        {/if}
