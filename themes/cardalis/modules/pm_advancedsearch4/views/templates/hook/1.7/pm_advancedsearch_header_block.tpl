...
{if empty($searchEngineTitle)}
	{assign var=searchEngineTitle value={l s='Filtrer par' mod='pm_advancedsearch4'}}
{/if}
	...
	<div id="PM_ASBlock_{$as_search.id_search|intval}" class="card">
		{if $searchEngineTitle}
			<div class="card-header">
				<span class="PM_ASBlockTitle d-md-block d-none">{l s='Filtrer par' mod='pm_advancedsearch4'}</span>
				...
				{/if} *}
				<span class="PM_ASBlockTitle d-md-none d-sm-block"><i class="fa fa-sliders" aria-hidden="true"></i> {l s='Filtrer' mod='pm_advancedsearch4'}</span>
			</div>
		{/if}
