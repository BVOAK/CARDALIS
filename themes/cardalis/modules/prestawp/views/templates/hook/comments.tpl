					...
					<a {if $pswp_blank}target="_blank"{/if} href="{$comment.url|escape:'html':'UTF-8'}">
                        ...
                        {if $pswp_show_content && isset($comment.comment_content)}
							<span class="comment-content">
                                {$comment.comment_content|strip_tags:false|escape:'html':'UTF-8'|html_entity_decode|truncate:200}
							</span>
                        {/if}
					</a>
				{else}
					<a {if $pswp_blank}target="_blank"{/if} href="{$comment.url|escape:'html':'UTF-8'}">
						{$comment.comment_author|escape:'html':'UTF-8'|truncate:200} <span class="comment-on">{l s='on' mod='prestawp'}</span> {$comment.post_title|escape:'html':'UTF-8'|truncate:200}
                        {if ($pswp_show_date)}
