...
{block name='step_content'}
  <div class="js-address-form">
    ...
    >
        ...
        {if $customer.addresses|count gt 1}
            {assign var="use_same_address" value=false}
        {/if}
      {if !$use_same_address}
        <h2 class="h4">{l s='Shipping Address' d='Shop.Theme.Checkout'}</h2>

          {else}
          <h2 class="h4">{l s='Adresse de livraison et de facturation' d='Shop.Theme.Checkout'}</h2>
      {/if}
      ...
      {elseif $customer.addresses|count > 0}
        ...
        {if $use_same_address && !$cart.is_virtual}
          <p>
            <a data-link-action="different-invoice-address" class="secondary-button" href="{$use_different_address_url}">
