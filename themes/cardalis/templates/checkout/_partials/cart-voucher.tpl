  ...
  <div class="block-promo">
    <div class="cart-voucher">
      ...
      <a class="collapse-button promo-code-button" data-toggle="collapse" href="#promo-code" aria-expanded="false" aria-controls="promo-code">
        <span class="hidden-lg-down">{l s='Have a promo code?' d='Shop.Theme.Checkout'}<i class="material-icons">&#xE313;</i></span>
        <span class="hidden-xl-up">{l s='promo code?' d='Shop.Theme.Checkout'}<i class="material-icons">&#xE313;</i></span>
      </a>
      <div class="promo-code collapse{if $cart.discounts|count > 0} in{/if}" id="promo-code">
        <form action="{$urls.pages.cart}" data-link-action="add-voucher" method="post" class="d-flex">
          <input type="hidden" name="token" value="{$static_token}">
          <input type="hidden" name="addDiscount" value="1">
          <input class="promo-input" type="text" name="discount_name" placeholder="{l s='Promo code' d='Shop.Theme.Checkout'}">
          <button type="submit" class="btn btn-default no-border ml-2"><span>{l s='Add' d='Shop.Theme.Actions'}</span></button>
        </form>
