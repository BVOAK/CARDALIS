...
{block name='page_title'}
  <div class="headerTitle">
    <h1 class="h1 mainTitle">{l s='Order history' d='Shop.Theme.Customeraccount'}</h1>
  </div>
{/block}
        ...
        {foreach from=$orders item=order}
          <tr>
            ...
            <td class="text-xs-center hidden-md-down">
               {*{assign var="sageFilename" value=OrderInvoice::getInvoiceSageFilename($order.details.id)}*}
              {* {if $sageFilename }
                <a href="/index.php?controller=pdf-invoice&id_order={$order.details.id}" download><i class="material-icons">&#xE415;</i></a>
              {else}
                -
              {/if} *}
    ...
    <div class="orders hidden-md-up">
      {foreach from=$orders item=order}
        <div class="order">
          <div class="row">
            <div class="col-10">
              <a href="{$order.details.details_url}">{l s="Order reference:"} {$order.details.id}</a>
              <div class="date">{l s="Date:"} {$order.details.order_date}</div>
              <div class="total">{l s="Total price:"} {$order.totals.total.value}</div>
              <div class="status">
                <span
                  class="label label-pill {$order.history.current.contrast}"
                  style="background-color:{$order.history.current.color}"
